    ...
    def post(self, request, pk):
        try:
            attendance = DrillAttendance.objects.get(
                drill_id=pk, crew_member=request.user
            )
...
class ComplianceView(APIView):
    ...
    def get(self, request):
        ...
        for ship in ships:
            ...
            overdue_tasks = (
                tasks.filter(due_date__lt=today).exclude(status="completed").count()
            )
            ...
            maintenance_score = (
                (completed_tasks / total_tasks * 100) if total_tasks else 100
            )
            drill_score = (
                (completed_drills / total_drills * 100) if total_drills else 100
            )
        ...
        return Response(
            {
                "fleet_score": (
                    round(sum(r["overall_score"] for r in result) / len(result), 1)
                    if result
                    else 0
                ),
                "ships": result,
